...
class DecisionTransformerGTController():
    def __init__(self,
                dt,
                rtg, 
                act_dim,
    ...
    def get_action_and_prediction(self):
        opoact_pred, curact = self.forward_pass(self.rewards_to_go,
                self.opo_acts, self.cur_acts, self.timesteps)
        opoact_pred = opoact_pred.argmax().item()
    ...
    def update(self, opoact, curact, reward):
        ...
        self.opo_acts[:, -2] = torch.tensor(opoact, **self.configs).reshape(
            self.n, self.opo_agents, self.act_dim)
        
        self.cur_acts = torch.cat([self.cur_acts, self.zero_cur_act], dim=1)
        self.cur_acts[:, -2] = torch.tensor(curact, **self.configs).reshape(
            self.n, self.cur_agents, self.act_dim)
    ...
    def reset_memory(self):
        self.rewards_to_go = torch.zeros([self.n, 1, 1], device = self.device, dtype=torch.float32)
        self.cur_acts = torch.zeros([self.n, 1, self.cur_agents, self.act_dim],
                                    device = self.device, dtype=torch.float32)
        self.opo_acts = torch.zeros([self.n, 1, self.opo_agents, self.act_dim],
                                    device = self.device, dtype=torch.float32)
        self.timesteps = torch.zeros([self.n, 1], device = self.device, dtype=torch.long)
        self.t = 0
    ...
    def forward_pass(self, target_returns, opo_acts, cur_acts, timesteps):
        # print("About to first forward")
        opoact_pred, _ = self.dt.get_action(
                                    target_returns, 
                                    opo_acts,
                                    cur_acts,
                                    timesteps
        ...
        if self.forward == "double":
            opo_acts[:, -1] = opoact_pred[:].argmax(dim=-1)
            _, curact_pred = self.dt.get_action(
                                    target_returns, 
                                    opo_acts,
                                    cur_acts,
                                    timesteps
